document.addEventListener("DOMContentLoaded", function () {
  ...
  const shuffleBtn = document.getElementById("shuffle-btn"); // ランダム再生ボタン
  let player = null;
  let currentVideoIndex = 0;
  let isVideoEnded = false; // 動画終了フラグ
  let isPlayerReady = false; // プレーヤーが準備完了しているかのフラグ
  let isShuffleMode = false; // ランダム再生モードのフラグ
    ...
    player = new YT.Player("yt-player", {
      ...
      events: {
        'onReady': function (event) {
          ...
          isPlayerReady = true;
          loadFirstVideo();
          setupFavoriteButtons(); // プレーヤーの準備完了後にボタンを設定
        },
  ...
  shuffleBtn.addEventListener("click", function () {
    isShuffleMode = !isShuffleMode; // モード切り替え
