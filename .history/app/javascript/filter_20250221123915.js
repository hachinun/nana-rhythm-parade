document.addEventListener("DOMContentLoaded", function () {
    ...
    function applyFavorites() {
        document.querySelectorAll(".video").forEach(video => {
            ...
            if (favoriteVideos[videoId]) {
                video.dataset.favorite = "true"; // `true`（Boolean）ではなく、"true"（文字列）を設定
            } else {
    ...
    if (showFavoritesBtn && showAllBtn) {
        showFavoritesBtn.addEventListener("click", function () {
            document.querySelectorAll(".video").forEach(video => {
                console.log(`Video ID: ${video.dataset.videoId}, data-favorite: ${video.dataset.favorite}`); // デバッグ用
                video.style.display = video.dataset.favorite === "true" ? "" : "none"; // 修正ポイント
            });
        });
