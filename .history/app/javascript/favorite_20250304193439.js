document.addEventListener("DOMContentLoaded", function () {
  ...
  function favoriteButtonClickHandler(event) {
    ...
    // ボタンの data-video-id を取得
    let videoId = this.dataset.videoId;
  
    // videoId の値を確認
    console.log(`📌 ボタンの videoId: ${videoId}`);
  
    // videoElement から取得する場合も確認
    let videoElement = this.closest(".video");
    let videoIdFromElement = videoElement.getAttribute("data-video-id");
    console.log(`📌 親要素 .video から取得した videoId: ${videoIdFromElement}`);
  
    // どちらの videoId を使用するか選択（この場合 videoIdFromElement を使用）
    let url = `/videos/${videoIdFromElement}/favorites/toggle`;
