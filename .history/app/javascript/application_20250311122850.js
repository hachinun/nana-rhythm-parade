...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function getNextVideo() {
    if (!window.filteredVideos || window.filteredVideos.length === 0) {
      // Case 1: filteredVideos is empty
      if (isRandomMode) {
        // Case 2: Random mode, pick a random video from videoItems
        return videoItems[Math.floor(Math.random() * videoItems.length)] || null;
      } else {
        // Case 3: No filteredVideos, return next from videoItems
        const nextIndex = currentVideoIndex + 1;
        return videoItems[nextIndex] || null;
      }
    } else {
      // Case 4: filteredVideos is not empty
      if (isRandomMode) {
        // Case 5: Random mode, pick a random video from filteredVideos
        return window.filteredVideos[Math.floor(Math.random() * window.filteredVideos.length)] || null;
      } else {
        // Case 6: Return next from filteredVideos
