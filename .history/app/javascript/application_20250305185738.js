...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (shuffleButton && !shuffleButton.hasAttribute('data-listener')) {
    shuffleButton.addEventListener("click", function () {
      isRandomMode = !isRandomMode;
      console.log(`ランダム再生モード: ${isRandomMode ? "ON" : "OFF"}`);
    });
    shuffleButton.setAttribute('data-listener', 'true');
  }

  // リピートボタン
  const repeatButton = document.querySelector("#repeat-btn");
  if (repeatButton && !repeatButton.hasAttribute('data-listener')) {
    repeatButton.addEventListener("click", function () {
      isRepeatMode = !isRepeatMode;
      console.log(`リピートモード: ${isRepeatMode ? "ON" : "OFF"}`);
    });
    repeatButton.setAttribute('data-listener', 'true');
  }

  // 再生/停止ボタン
  const playButton = document.querySelector("#play-btn");
  if (playButton && !playButton.hasAttribute('data-listener')) {
    ...
    playButton.setAttribute('data-listener', 'true');
  }
  ...
  if (nextButton && !nextButton.hasAttribute('data-listener')) {
    nextButton.addEventListener("click", function () {
      const filteredVideos = getFilteredVideoItems();
      const nextIndex = filteredVideos.indexOf(filteredVideos.find(item => item.getAttribute('data-video-id') === videoItems[currentVideoIndex].getAttribute('data-video-id'))) + 1;
  
      if (nextIndex < filteredVideos.length) {
        currentVideoIndex = Array.from(videoItems).indexOf(filteredVideos[nextIndex]);
        console.log(`⏭️ 次の動画へ: ${filteredVideos[nextIndex].getAttribute('data-video-id')}`);
        filteredVideos[nextIndex].click();
      } else {
        console.log("🚫 次の動画がありません。");
      }
    });
  ...
  if (prevButton && !prevButton.hasAttribute('data-listener')) {
    prevButton.addEventListener("click", function () {
      const filteredVideos = getFilteredVideoItems();
      const prevIndex = filteredVideos.indexOf(filteredVideos.find(item => item.getAttribute('data-video-id') === videoItems[currentVideoIndex].getAttribute('data-video-id'))) - 1;
  
      if (prevIndex >= 0) {
        currentVideoIndex = Array.from(videoItems).indexOf(filteredVideos[prevIndex]);
        console.log(`⏮️ 前の動画へ: ${filteredVideos[prevIndex].getAttribute('data-video-id')}`);
        filteredVideos[prevIndex].click();
      } else {
