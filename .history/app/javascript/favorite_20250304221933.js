document.addEventListener("DOMContentLoaded", function () {
  ...
  function initializeButtonState(button) {
    ...
    console.log(`🎯 ボタン初期化: button videoId=${videoId}, isFavorite=${isFavorite}`);
  ...
  function favoriteButtonClickHandler(event) {
    ...
    console.log(`📌 ボタンの videoId: ${videoId}`);  // ボタンのvideoIdを確認
    
    let videoElement = this.closest(".video");
    let parentVideoId = videoElement.getAttribute("data-video-id");
    console.log(`📌 親要素 .video から取得した videoId: ${parentVideoId}`);  // 親要素から取得したvideoIdを確認
